...
apiview.config(["$httpProvider", function (a) {
    ...
    a.when("/project/all", {
        ...
        templateUrl: "/Tpl/Setting/advance.html",
        page_title: "高级设置",
        controller: "ProjectController"
    }).when("/setting/export", {
    ...
    a.user_data = {}, a.project_list = [], a.project_count = [], a.getProfile = function () {
        ...
        a.commonAjax("post", "/Project/create.html", function (a) {
            window.location.href = "/p/" + a.data.project_id + "#/project/dashbord"
        })
    ...
    a.getProjectInfo = function () {
        b.get("/Project/info?pid=" + pid).success(function (b) {
            a.project_info = b.data
        })
    }, a.getActivity = function () {
        b.get("/Project/activity?pid=" + pid).success(function (b) {
            a.activity = b.data
        })
    ...
    a.group_data = {}, a.group_list = [], a.getGroupList = function () {
        ...
        b.post("/group/create?pid=" + pid, $.param(a.group_data)).success(function (b) {
    ...
    a.api_list = [], a.api_list_origin = [], a.api_data = {
        ...
        var c = "apis_hash_" + pid, d = "apis_list_" + pid, e = localStorage.getItem(c);
        e && (a.api_list = JSON.parse(localStorage.getItem(d))), b.get("/api/getList?" + $.param({pid: pid})).success(function (b) {
            var g = f.createHash(JSON.stringify(b));
            e && e === g || (a.api_list = b.data, localStorage.setItem(d, JSON.stringify(b.data)), localStorage.setItem(c, g))
        })
    }, a.getApiInfo = function () {
        b.get("/api/getApi?" + $.param({pid: pid, id: c.id})).success(function (b) {
            true==b.success ? (a.api_info = b.data,a.api_info.params=Json.parse(a.api_info.params),a.apiParseOutput(b.data.output)) : av.alert("error", "错误！", "接口ID错误")
        })
    }, a.$watch("search_keyword", function (b, c, d) {
        function e(a) {
            return f.test(a.name) || f.test(a.brief)
        }

        if (c ? a.api_list = angular.copy(a.api_list_origin) : a.api_list_origin = angular.copy(a.api_list), b) {
            b = b.replace(/\//, "");
            for (var f = new RegExp(b, "i"), g = 0; g < a.api_list.length; g++) {
                var h = a.api_list[g].apis;
                a.api_list[g].apis = h.filter(e(api))
            }
        }
    }), a.apiEdit = function () {
        ...
        return $("form").validator("validate"), $("form").data("bs.validator") && $("form").data("bs.validator").hasErrors() ? !1 : /格式错误/.test($("#jsonoutput").html()) ? !1 : (a.api_data.pid = pid, a.api_data.params = angular.toJson(a.api_data.params), void b.post("/api/create", $.param(a.api_data)).success(function (b) {
